...
def read_config_vpn():
	# def read_current_list_vpn():
	with open('last_working_config','r') as file:
		lines = file.readlines()
		print(str(len(lines)))
		print(str(lines))
		if "locked" in lines[0]:
			final_vpn,random_vpn=get_random_vpn()
			cmf='echo "'+final_vpn+'" > last_working_config'
			os.system(cmf)
			ok=final_vpn
		else :
			final_vpn=lines[0].replace('\n','')
		
	return final_vpn


aw_vpn=read_config_vpn()
# aw_vpn="/root/VPN/conf/ipvanish-US-Atlanta-atl-b45.ovpn"
...
def fnc_vpn():
	...
	x = subprocess.Popen(['openvpn', '--auth-nocache', '--config',aw_vpn , '--log' , '/var/log/openvpn/openvpn.log'])
